...
public final class TokenHandler {
	...
	public SSOUser parseUserFromToken(String token) {
		...
		if (parts.length == 2 && parts[0].length() > 0
			...
			try {
				...
				if (validHash) {
					final SSOUser user = fromJSON(userBytes);
	...
	public String createTokenForUser(SSOUser user) {
	...
	private SSOUser fromJSON(final byte[] userBytes) {
		
		try {
			return new ObjectMapper().readValue(
					new ByteArrayInputStream(userBytes), SSOUser.class);
		} catch (IOException e) {
			throw new IllegalStateException(e);
		}
	}
	
	private byte[] toJSON(SSOUser user) {
