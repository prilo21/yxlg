...
(function($){
	...
	function load(target, data, isHierarchy){
		...
		function _hierarchyLoad(data){
			var form = $(target);
			var nameMap = {};
			_recursiveFiles(data, "", nameMap);
			data = nameMap;
			for(var name in data){
				var val = data[name];
				if (!_checkField(name, val)){
		 ...
		 * @private
		 */
		function _recursiveFiles(data, name, nameMap) {
			if(typeof data == 'object' && data != null){
				for(var n in data){
					//闭包保持状态
					(function(name){
						name = name?(name + '.' + n):n;
						_recursiveFiles(data[n], name, nameMap);
					})(name);
				}
			}else {
				nameMap[name] = data;
	...
	$.extend($.fn.form.methods, {
		...
		/**
		 * form数据层级加载 example: order.good.name
		 * @param jq
		 * @param data
		 * @returns {*}
		 * @author yu.tom
		 */
		hierarchyLoad:function (jq, data) {
			return jq.each(function(){
				load(this, data, true);
			});
		}
